...
local dsw = machine.ioport.ports[":DSW"]
local function set_freeplay()
  for _, f in pairs(dsw.fields) do
    if f.name == "Coin A" or f.name == "Coin B" then f.user_value = 0 end
  end
end
local last_region = ""
local pressed = false
...
emu.register_frame_done(function()
  frame = frame + 1
  if frame == 2 then set_freeplay() end
