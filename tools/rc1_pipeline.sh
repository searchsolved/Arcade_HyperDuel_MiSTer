...
# for a fresh RBF, STA-gate, stage as builds/Hyprduel_rc1.rbf.
...
DEST=$REPO/builds/Hyprduel_rc1.rbf
...
for i in $(seq 1 40); do
  ...
  if [ "$MT_EPOCH" -gt "$FIRED_EPOCH" ]; then
    ...
    $SSHPC "cmd /c type C:\\hyprduel\\mister\\output_files\\Hyprduel.sta.summary" | tr -d '\r' > /tmp/sta_rc1.summary
    grep -E "Type|Slack" /tmp/sta_rc1.summary | paste - - | head -4
    NEG=$(grep -E "^ *Slack" /tmp/sta_rc1.summary | awk '{print $3}' | grep -c "^-" || true)
    if [ "$NEG" -ne 0 ]; then
